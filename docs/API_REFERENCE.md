...
- **路径**: `POST /api/service-orders/guest`
- **认证**: ❌ 无需认证（游客/顾客扫码进入时可用）
- **参数**:
  ```json
  {
    "table_no": "普台1",
...
- **返回**:
  ...
  {
    "success": true,
    "data": { "id": 41, "status": "待处理" }
  }
  ```
- **说明**: 
  - 专为H5"邀请助教上桌"功能设计，游客（未登录顾客）可直接提交
  - `requester_name` 和 `requester_type` 自动设为 "顾客"
  - `requirement` 格式：`助教上桌邀请函（工号 艺名）`
  - `coach_no` 可选，用于记录邀请的助教
  - 新增日期：2026-04-24
...
- **路径**: `GET /api/system-config/service-categories`
- **参数**: 无
- **认证**: 无需认证
- **返回**: 
  ```json
    ...
    "data": [
      {
        "label": "酒水冰块",
        "color": "#3498db",
        "tags": ["啤酒杯", "加冰块", "食用冰桶", "温水", "冰杯"]
      },
      {
        "label": "器具用品",
        "color": "#e67e22",
        "tags": ["换垃圾袋", "筷子/碗", "纸巾", "毛毯", "打火机", "吸管", "烟灰缸", "骰子"]
      },
      {
        "label": "人工服务",
        "color": "#e74c3c",
        "tags": ["音响连接", "看账单", "拿外卖", "搞卫生", "挂烟1包", "挂烟2包"]
      }
    ]
  }
  ```
- **缓存**: 服务端内存缓存 1 小时
- **说明**: 获取服务下单页面的分类按钮配置。配置存储在 `system_config` 表，key 为 `service_order_categories`。前端也缓存 1 小时。
