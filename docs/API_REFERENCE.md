...
- **路径**: `POST /api/lejuan-records/:id/return`
- **认证**: 需要后台权限（coachManagement）
- **请求体**: `{ "operator": "admin_username" }`
- **返回**: `{ "success": true, "data": { "lejuan_hours": 2 } }`
- **说明**: 计算外出小时数（向上取整），恢复水牌为空闲状态
