...
async function repairData() {
  // 1. 遍历所有有 device_fingerprint 但没有对应会员的订单
  const orders = await dbAll(`
    SELECT DISTINCT device_fingerprint
    FROM orders
    WHERE device_fingerprint IS NOT NULL
      AND device_fingerprint != ''
      AND member_phone IS NULL OR member_phone = ''
  `);
  
  let repaired = 0;
  for (const order of orders) {
    // 2. 查找该 device_fingerprint 是否已有会员
    ...
    if (!existingMember) {
      // 3. 查找助教 phone（通过其他途径获取，如手动录入或关联查询）
      // 此处假设通过订单的其他信息（如备注）或手动指定助教
      // 实际实现时可能需要更复杂的逻辑
      continue;
    }
  }
  
  // 4. 另一种修复方式：通过助教手机号找到会员，补充 device_fingerprint
  ...
  for (const coach of coaches) {
    ...
    if (member && !member.device_fingerprint) {
      // 查找该会员的订单，获取 device_fingerprint
      const coachOrder = await dbGet(
        'SELECT device_fingerprint FROM orders WHERE member_phone = ? AND device_fingerprint IS NOT NULL LIMIT 1',
        [coach.phone]
      );
      
      if (coachOrder && coachOrder.device_fingerprint) {
        // 写入 device_fingerprint 到 members 表
        await enqueueRun(
          'UPDATE members SET device_fingerprint = ? WHERE member_no = ?',
          [coachOrder.device_fingerprint, member.member_no]
        );
        repaired++;
      }
    }
