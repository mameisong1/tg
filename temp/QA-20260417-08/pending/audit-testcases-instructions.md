...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats?period=yesterday" \
...
```
- `coaches` 数组按 `missing_count` 倒序排序
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats?period=day_before_yesterday" \
  -H "Authorization: Bearer $TOKEN" | python3 -m json.tool
```

**预期结果**:
- HTTP 200
- `period` 为 `"day_before_yesterday"`
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats?period=current_month" \
  -H "Authorization: Bearer $TOKEN" | python3 -m json.tool
```

**预期结果**:
- HTTP 200
- `period` 为 `"current_month"`
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats?period=last_month" \
  -H "Authorization: Bearer $TOKEN" | python3 -m json.tool
```
...
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=yesterday&coach_no=10078" \
  -H "Authorization: Bearer $TOKEN" | python3 -m json.tool
```
...
```bash
# 先查一个有效 coach_no
sqlite3 /TG/run/db/tgservice.db "SELECT coach_no, stage_name FROM coaches WHERE coach_no=10002;"
# 预期: 10002|陆飞

# 创建上桌单（使用当前时间往回推，确保下桌单能在15小时内发出）
TABLE_IN_ID=$(sqlite3 /TG/run/db/tgservice.db "
...
coaches = data.get('data',{}).get('coaches',[])
...
```bash
TABLE_IN_ID2=$(sqlite3 /TG/run/db/tgservice.db "
INSERT INTO table_action_orders (table_no, coach_no, order_type, stage_name, status, created_at, updated_at)
...
```bash
curl -s -w "\nHTTP_CODE:%{http_code}\n" -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats?period=yesterday" \
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=current_month&coach_no=10012" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "
import sys, json
data = json.load(sys.stdin)
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=current_month&coach_no=10023" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "
import sys, json
data = json.load(sys.stdin)
details = data.get('data',{}).get('details',[])
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=current_month&coach_no=10021" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "
import sys, json
data = json.load(sys.stdin)
details = data.get('data',{}).get('details',[])
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats?period=2026-04-15" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "
import sys, json
data = json.load(sys.stdin)
coaches = data.get('data',{}).get('coaches',[])
...
```bash
curl -s -w "\nHTTP_CODE:%{http_code}\n" -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats?period=invalid_period" \
  -H "Authorization: Bearer $TOKEN"
```

**预期结果**: HTTP 400，返回参数错误提示，如 `"error": "无效的周期参数，应为: yesterday, day_before_yesterday, current_month, last_month"`
...
```bash
curl -s -w "\nHTTP_CODE:%{http_code}\n" -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -s -w "\nHTTP_CODE:%{http_code}\n" -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=yesterday" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=yesterday&coach_no=99999999" \
  -H "Authorization: Bearer $TOKEN" | python3 -m json.tool
```
...
**预期结果**: HTTP 200，`coaches` 为空数组或 `total_missing` 为 0
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=current_month&coach_no=10025" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "
import sys, json
data = json.load(sys.stdin)
details = data.get('data',{}).get('details',[])
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=current_month&coach_no=10026" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "
import sys, json
data = json.load(sys.stdin)
details = data.get('data',{}).get('details',[])
...
```bash
curl -s -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-detail?period=current_month&coach_no=10032" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "
import sys, json
data = json.load(sys.stdin)
details = data.get('data',{}).get('details',[])
...
for i in $(seq 1 10); do
  curl -s -o /dev/null -w "%{time_total}\n" \
    -X GET "http://127.0.0.1:8088/api/table-action-orders/missing-stats?period=current_month" \
    -H "Authorization: Bearer $TOKEN"
...
```bash
sqlite3 /TG/run/db/tgservice.db "EXPLAIN QUERY PLAN
...
```bash
sqlite3 /TG/run/db/tgservice.db ".indices table_action_orders"
```
...
# 清理测试数据
sqlite3 /TG/run/db/tgservice.db "DELETE FROM table_action_orders WHERE stage_name='测试';"
```
...
以下数据来自生产数据库 `/TG/run/db/tgservice.db`，可直接用于验证：
