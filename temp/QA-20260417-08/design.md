...
> **注意**：实际实现中，统计接口直接使用 SQL `GROUP BY` + `COUNT(*)` 返回聚合结果，
> 不先查出所有明细再在 JS 中聚合。明细查询仅在用户点击统计数据时触发。

### 6.2 统计聚合 SQL（直接 SQL 聚合，不返回明细）
...
**说明**：用户点击统计数据时，前端将 `coach_no`（隐藏字段）传给 API，后端按此条件查询该助教的缺失明细。

### 6.4 周期计算（后端使用 TimeUtil）
