...
# 登录获取cookie
echo "🔑 登录中..."
LOGIN_RESP=$(curl -s -c /tmp/qa5_cookie.txt -X POST "$BASE/api/admin/login" \
...
# 通用API请求函数
api_get() {
  curl -s -b /tmp/qa5_cookie.txt "$BASE$1"
}
api_post() {
  curl -s -b /tmp/qa5_cookie.txt -X POST "$BASE$1" -H "Content-Type: application/json" -d "$2"
}
api_put() {
  curl -s -b /tmp/qa5_cookie.txt -X PUT "$BASE$1" -H "Content-Type: application/json" -d "$2"
}
api_delete() {
  curl -s -b /tmp/qa5_cookie.txt -X DELETE "$BASE$1"
}
db_query() {
  $DB_CMD "$1" 2>/dev/null
}
...
echo "  更新后: coaches.status=${C_STATUS:-无} water_boards=$W_COUNT"
...
echo "  更新后: coaches.status=${C_STATUS:-无} water_boards.status=${W_STATUS:-无}"

if [ "$C_STATUS" = "全职" ] && [ -n "$W_STATUS" ]; then
  add_result "TC-03" "PASS" "status=全职, water_boards已创建(status=$W_STATUS)"
else
  add_result "TC-03" "FAIL" "status=${C_STATUS:-无}(预期全职), water_boards=${W_STATUS:-无}(预期有记录)"
fi
...
if [ "$C_STATUS" = "兼职" ] && [ -n "$W_STATUS" ]; then
  ...
  add_result "TC-04" "FAIL" "status=${C_STATUS:-无}(预期兼职), water_boards=${W_STATUS:-无}(预期有记录)"
fi
...
W_STATUS=$($DB_CMD "SELECT wb_status FROM water_boards WHERE coach_no='$COACH_NO';" 2>/dev/null)
echo "  更新后: coaches.shift=${C_SHIFT:-无} water_boards.status=${W_STATUS:-无}"
