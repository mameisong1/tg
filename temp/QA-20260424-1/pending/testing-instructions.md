...
### 3.4 服务单创建API（游客公开API）

⚠️ **重要说明**：顾客扫码进入时可能未登录，因此需要使用公开API `POST /api/service-orders/guest`

#### TC-API-006: 游客创建"邀请上桌"服务单（无需登录）
**优先级**: P0 ⭐ 核心
**目的**: 验证游客（未登录顾客）可以创建邀请助教上桌的服务单
**前置条件**: 
...
```bash
# 游客直接创建服务单（无需token）
curl -s -X POST http://127.0.0.1:8088/api/service-orders/guest \
...
2. ✅ 返回新创建的服务单ID
3. ✅ 服务单状态为"待处理"
4. ✅ requester_type 自动设为 "顾客"
5. ✅ 数据库中正确记录服务单内容

**数据库验证**:
```bash
sqlite3 /TG/run/db/tgservice.db "
SELECT id, table_no, requirement, requester_type, status, created_at
FROM service_orders
...
| TC-API-006 | P0 | 待执行 | 游客创建服务单（公开API） | ⭐ 核心测试 |
| TC-API-007 | P1 | 待执行 | 创建服务单-缺台桌 |
| TC-API-008 | P1 | 待执行 | 创建服务单-缺需求 |
| TC-API-009 | P1 | 待执行 | 创建服务单-未登录（原API） | 公开API无需此测试 |
...
## 输出要求
- 测试结果写入：/TG/temp/QA-20260424-1/test-results.md
