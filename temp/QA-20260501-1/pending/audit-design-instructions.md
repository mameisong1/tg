...
| `/TG/tgservice/backend/routes/lejuan-records.js` | 修改 return API：强制使用钉钉打卡时间 |
| `/TG/tgservice/backend/services/dingtalk-service.js` | 新增 `queryAttendanceWithRetry` 轮询查询方法 |
...
**变更内容**：
- 同 clock-in API，强制使用钉钉打卡时间
- `return_time = dingtalk_return_time`
...
关闭沙漏弹框，显示失败提示
「打卡失败：未获取到钉钉打卡时间，请联系助教管理或店长手动打卡翻牌」
```
...
前端显示提示+勾选框（同上班打卡）
    ↓
调用 POST /api/dingtalk-attendance/query (clock_type=return)
    ↓
同上班打卡流程...
```
