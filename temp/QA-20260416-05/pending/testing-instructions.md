...
**预期结果**：
- 人气值 = 1（不变。设计规则：只在首次加入购物车时+1，同一商品重复加购不重复+1）
...
**预期结果**：
- 苏打水 popularity = 1（清空购物车不减人气值，用户已确认）
- 茶兀 popularity = 1（清空购物车不减人气值，用户已确认）
...
**预期结果**：
- 人气值 = 1（设计规则：首次加车+1，不论quantity是多少）
...
## 输出要求
- 测试结果写入：/TG/temp/QA-20260416-05/test-results.md
