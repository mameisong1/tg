...
async function apiRequest(url, method = 'GET', data = null) {
  ...
  return new Promise((resolve, reject) => {
    uni.request({
      ...
      fail: (err) => {
        errorReporter.track('ac_api_request_fail', { url, method, error: err?.message || String(err) })
...
async function loadTables() {
  ...
  } catch (e) { console.error('[空调台桌列表] 加载失败', e) }
}
...
function selectTable(table) {
  if (!table.switches || table.switches.length === 0) {
    uni.showToast({ title: '该台桌未关联空调设备', icon: 'none' })
    return
  }
...
async function tableControlAction(action) {
  ...
  closeTableConfirm()
  try {
    await apiRequest('/ac/table-control', 'POST', {
      table_name_en: table.table_name_en,
      action
    })
    uni.showToast({ title: `${table.table_name_cn} ${action === 'ON' ? '开空调' : '关空调'}成功`, icon: 'success' })
  } catch (e) {
    errorReporter.track('ac_tableControl_error', { table_name_en: table.table_name_en, action, error: e?.message || String(e) })
...
async function toggleAutoOff() {
  ...
  confirmText.value = `确认${action}智能省电-自动（空调）？`
...
function executeManualOff() {
  ...
  confirmText.value = '确认执行一次智能省电（手动）？将关闭当前空闲台桌的空调。'
  pendingAction = 'manualOff'
  showConfirm.value = true
}

function executeScene(scene) {
  confirmText.value = `确认执行场景"${scene.scene_name}"？（${scene.action === 'ON' ? '开空调' : '关空调'}）`
...
function labelControl(action) {
  ...
  confirmText.value = `确认对标签"${selectedLabel.value}"执行${action === 'ON' ? '开空调' : '关空调'}？`
...
async function confirmAction() {
  ...
  try {
    if (action === 'toggleAutoOff') {
      await apiRequest('/ac/auto-off-toggle', 'POST')
      ...
      const res = await apiRequest('/ac/auto-off-manual', 'POST')
      const count = res?.turnedOffCount || 0
      uni.showToast({ title: `智能省电执行完成，实际关空调 ${count} 个`, icon: 'success', duration: 3000 })
    } else if (action.type === 'scene') {
      const scene = action.scene
      if (!scene || !scene.id) {
        throw new Error('场景数据无效')
      }
      await apiRequest(`/ac/scene/${scene.id}`, 'POST')
    ...
    errorReporter.track('ac_confirmAction_error', { pendingAction: JSON.stringify(action), error: e?.message || String(e) })
    uni.showToast({ title: e.message || '操作失败', icon: 'none' })
  }
}
...
.card-header { display: flex; align-items: center; gap: 8px; padding: 16px 16px 0; }
...
.toggle-switch { width: 48px; height: 28px; border-radius: 14px; background: rgba(255,255,255,0.15); position: relative; transition: background 0.3s; }
.toggle-switch.active { background: rgba(34,197,94,0.5); }
.toggle-thumb { width: 24px; height: 24px; border-radius: 12px; background: #fff; position: absolute; top: 2px; left: 2px; transition: transform 0.3s; }
...
.area-btn { flex-shrink: 0; padding: 6px 14px; border-radius: 16px; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.1); font-size: 13px; color: rgba(255,255,255,0.7); box-sizing: border-box; }
.area-btn.active { background: rgba(218,165,32,0.25); border-color: rgba(218,165,32,0.5); color: #d4af37; }
.area-btn text { white-space: nowrap; }

/* 台桌网格 */
.table-grid { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; }
.table-btn { padding: 10px 12px; border-radius: 8px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); flex: 1; min-width: calc(25% - 6px); max-width: calc(25% - 6px); text-align: center; overflow: hidden; box-sizing: border-box; }
...
/* 标签选择器 */
.label-picker { display: flex; align-items: center; justify-content: space-between; padding: 12px 16px; background: rgba(255,255,255,0.05); border-radius: 10px; border: 1px solid rgba(255,255,255,0.1); box-sizing: border-box; }
...
.action-btn { flex: 1; padding: 14px; border-radius: 10px; text-align: center; font-size: 15px; font-weight: 500; box-sizing: border-box; }
...
.scene-btn-top { flex: 1; min-width: 0; max-width: calc(33.33% - 6px); padding: 10px 6px; border-radius: 8px; display: flex; flex-direction: column; align-items: center; gap: 4px; transition: all 0.2s; overflow: hidden; }
...
.scene-on { background: rgba(218,165,32,0.15); border: 1px solid rgba(218,165,32,0.3); }
.scene-off { background: rgba(100,100,150,0.15); border: 1px solid rgba(100,100,150,0.3); }

/* 快捷场景（底部） */
.scene-card-bottom { border-color: rgba(255,255,255,0.1); background: rgba(15,15,22,0.7); }
...
.scene-btn-bottom { flex: 1; min-width: calc(50% - 4px); padding: 10px 8px; border-radius: 8px; display: flex; flex-direction: column; align-items: center; gap: 4px; transition: all 0.2s; }
...
/* 确认弹窗 */
.confirm-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.7); z-index: 200; display: flex; align-items: center; justify-content: center; }
.confirm-box { background: rgba(20,20,30,0.95); border-radius: 16px; padding: 24px; width: 85%; max-width: 340px; border: 1px solid rgba(218,165,32,0.2); }
...
.confirm-btn { flex: 1; padding: 12px; border-radius: 10px; text-align: center; font-size: 15px; }
