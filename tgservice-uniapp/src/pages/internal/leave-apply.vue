  ...
  try {
    ...
    uni.chooseImage({ count: 1, sizeType: ['compressed'], sourceType: ['album', 'camera'],
      success: async (res) => {
        ...
        uni.uploadFile({ url: sigRes.uploadUrl, filePath: res.tempFilePaths[0], name: 'file',
          success: () => { form.value.proof_image_url = sigRes.fileUrl; uni.hideLoading(); uni.showToast({ title: '上传成功', icon: 'success' }) },
