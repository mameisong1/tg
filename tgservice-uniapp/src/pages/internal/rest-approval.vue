...
// 判断是否可撤销：当前时间 < 休息日期 12:00
const canCancel = (item) => {
  const restDate = getRestDate(item)
  if (!restDate || restDate === '-') return false

  const deadline = new Date(restDate + ' 12:00:00+08:00')
  const now = new Date()
...
const cancelApproved = async (item) => {
  ...
  uni.showModal({
    title: '确认撤销',
    content: `确定撤销 ${item.stage_name} 的休息申请（${restDate}）？`,
    success: async (res) => {
      if (res.confirm) {
        try {
